    ...
    <swiper :indicator-dots="true" :autoplay="true" :interval="3000" :duration="1000" :circular="true">
      ...
      <swiper-item v-for="(item, i) in banners" :key="i">
        <view class="swiper-item">
          <!-- 动态绑定图片的 src 属性 -->
          <image @click="goGoodsDetails(item.goods_id)" :src="item.image_src"></image>
        </view>
      </swiper-item>
    </swiper>
    <!-- 分类导航区域 -->
        ...
        <view class="floor-img-box">
          ...
          <view class="right-img-box">
            <view class="right-img-item" v-for="(item2, i) in item.product_list" :key="i" v-if="i !==0">
              <image @click="goGoodsList(item2.navigator_url)" class="image" :src="item2.image_src"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goGoodsDetails(id) {
        uni.navigateTo({
          url: '/subpack/goods_details/goods_details?id=' + id
        })
      },
      async loadNavs() {
      ...
      goGoodsList(url) {
        uni.navigateTo({
          url: '/subpack/goods_list/goods_list?' + url.split("?")[1]
        })
      }
    },
